...
## JSHint — Détecteur d'erreurs JavaScript

**JSHint** est un **linter** : un outil qui analyse le code JavaScript et signale les erreurs, les mauvaises pratiques et les incohérences **avant même d'exécuter le code**.

### Pourquoi un linter ?

Sans linter, les erreurs ne sont visibles qu'à l'exécution dans le navigateur. Avec JSHint, WebStorm souligne les problèmes en temps réel pendant qu'on tape, comme un correcteur orthographique pour le code.

Exemples de ce que JSHint détecte :
- Variable utilisée sans avoir été déclarée
- `==` au lieu de `===` (comparaison stricte)
- Variable déclarée mais jamais utilisée
- `use strict` manquant

### Configuration `.jshintrc`

Le fichier `.jshintrc` à la racine du projet configure les règles JSHint. Voici les options de notre projet :

```json
{
  // --- ENVIRONNEMENTS ---
  "browser": true,   // Autorise les globales navigateur (document, window, console)
  "jquery": true,    // Autorise le $ de jQuery (pas utilisé ici, mais pas gênant)
  "node": true,      // Autorise les globales Node.js (require, module)
  "esversion": 6,    // Active la syntaxe ES6 (let, const, arrow functions, template literals)
  "devel": true,     // Autorise console.log(), prompt(), alert() sans avertissement

  // --- RÈGLES STRICTES ---
  "camelcase": true,  // Impose le camelCase pour les noms de variables (maVariable, pas ma_variable)
  "eqeqeq": true,    // Interdit == et !=, oblige === et !== (comparaison stricte)
  "indent": 2,       // Indentation de 2 espaces
  "latedef": "nofunc",// Interdit d'utiliser une variable avant sa déclaration (sauf fonctions)
  "maxlen": 100,     // Longueur max d'une ligne : 100 caractères
  "newcap": true,    // Les constructeurs doivent commencer par une majuscule (new Personne())
  "quotmark": "double",// Impose les guillemets doubles pour les strings
  "strict": true,    // Exige "use strict" en haut du fichier
  "undef": true,     // Interdit les variables non déclarées
  "unused": true     // Avertit si une variable est déclarée mais jamais utilisée
}
```

### Activer JSHint dans WebStorm

1. **File** → **Settings** (ou **Preferences** sur Mac)
2. **Languages & Frameworks** → **JavaScript** → **Code Quality Tools** → **JSHint**
3. Cocher **Enable**
4. Sélectionner **Use config files** (WebStorm trouvera automatiquement le `.jshintrc`)
5. Cliquer **OK**

Les erreurs apparaissent directement dans l'éditeur avec des soulignements jaunes/rouges.

---

## Déboguer JavaScript avec WebStorm

Le débogueur WebStorm permet de **mettre en pause** l'exécution du code, d'**inspecter les variables** et d'avancer **ligne par ligne**. Plus puissant que `console.log()` pour comprendre ce qui se passe.

### Mettre un point d'arrêt (breakpoint)

1. Cliquer dans la **gouttière** (marge gauche) à côté du numéro de ligne → un point rouge apparaît
2. Le code s'arrêtera à cette ligne quand il sera exécuté

### Lancer le débogueur

1. Ouvrir `index.html`
2. Cliquer sur l'icône navigateur en haut à droite **tout en maintenant la touche `Shift` enfoncée**
   → WebStorm ouvre le navigateur **avec le débogueur connecté**
3. Alternative : clic droit sur `index.html` → **Debug 'index.html'**

### Naviguer dans le code en pause

Quand le code atteint un breakpoint, la barre d'outils du débogueur apparaît :

| Bouton | Raccourci | Action |
|--------|-----------|--------|
| **Resume** | `F9` | Reprend l'exécution jusqu'au prochain breakpoint |
| **Step Over** | `F8` | Exécute la ligne courante, passe à la suivante |
| **Step Into** | `F7` | Entre dans la fonction appelée sur cette ligne |
| **Step Out** | `Shift+F8` | Sort de la fonction courante, revient à l'appelant |

### Inspecter les variables

Quand le code est en pause :
- **Onglet Variables** : affiche toutes les variables locales et leurs valeurs
- **Survol** : passer la souris sur une variable dans le code affiche sa valeur
- **Onglet Console** : taper une expression pour l'évaluer dans le contexte actuel

### Exemple pratique avec CinéJS

1. Mettre un breakpoint dans la boucle `for...of` de `afficherFilmsConsole`
2. Lancer le débogueur
3. Observer la variable `film` qui change à chaque itération
4. Utiliser **Step Over** (`F8`) pour avancer film par film
